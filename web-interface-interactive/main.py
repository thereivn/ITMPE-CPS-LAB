...
if 'smooth_surface' not in st.session_state:
    st.session_state.smooth_surface = True
...
def validate_data_ranges(P1, P2, P3, alpha):
    """Проверка и объяснение диапазонов данных с учетом погрешности"""
    ranges_info = f"α={alpha}: P1∈[{P1.min():.2f}, {P1.max():.2f}], P2∈[{P2.min():.2f}, {P2.max():.2f}], P3∈[{P3.min():.2f}, {P3.max():.2f}]"
    
    # Расчет теоретических диапазонов с учетом погрешности
    sigma1 = 4 / np.sqrt(12)  # 1.155
    sigma2 = 2                # 2.0
    sigma3 = 3                # 3.0
    
    # Теоретические диапазоны с погрешностью 3σ (99.7% значений)
    expected_P1 = f"[{-3*alpha*sigma1:.1f}, {4 + 3*alpha*sigma1:.1f}]"
    expected_P2 = f"[{-3*alpha*sigma2:.1f}, {8 + 3*alpha*sigma2:.1f}]"  # 8 - практический максимум Эрланга(4,1)
    expected_P3 = f"[{-9 - 3*alpha*sigma3:.1f}, {9 + 3*alpha*sigma3:.1f}]"
    
    explanation = f"""
    **Объяснение для α={alpha}:**
    - Без погрешности: P1∈[0,4], P2∈[0,8], P3∈[-9,9]
    - С погрешностью ±{3*alpha:.1f}σ: P1∈{expected_P1}, P2∈{expected_P2}, P3∈{expected_P3}
    - Фактические данные находятся в ожидаемых пределах с учетом погрешности измерений
    """
    
    return ranges_info, explanation
...
def select_polynomial_degree(X, y, max_degree=2):
    ...
    for degree in range(2, max_degree + 1):
        ...
        if (rss_prev - rss_curr) > 0:
            ...
            if significant and (rss_prev - rss_curr) / rss_prev > 0.01:  # Минимальное улучшение 1%
                best_degree = degree
            else:
                break
        else:
...
def create_3d_regression_plot(P1, P2, P3, model, alpha, degree, grid_size=100, smooth_surface=True):
    """
    Создание 3D визуализации регрессионной поверхности
    При отключенном сглаживании добавляем неровности к полиномиальной поверхности
    """
    # Создаем детальную сетку для предсказаний
    P1_min, P1_max = P1.min(), P1.max()
    P2_min, P2_max = P2.min(), P2.max()
    
    # Увеличиваем сетку для лучшей детализации
    P1_range = np.linspace(P1_min, P1_max, grid_size)
    P2_range = np.linspace(P2_min, P2_max, grid_size)
    P1_grid, P2_grid = np.meshgrid(P1_range, P2_range)
    
    # Всегда используем регрессионную модель для основной поверхности
    grid_points = np.column_stack((P1_grid.ravel(), P2_grid.ravel()))
    P3_pred_grid = model.predict(grid_points).reshape(P1_grid.shape)
    
    if not smooth_surface:
        # ДОБАВЛЯЕМ НЕРОВНОСТИ К ПОЛИНОМИАЛЬНОЙ ПОВЕРХНОСТИ
        # Вычисляем остатки (разницу между реальными данными и моделью)
        predicted_values = model.predict(np.column_stack((P1, P2)))
        residuals = P3 - predicted_values
        
        # Интерполируем остатки на сетку
        points = np.column_stack((P1, P2))
        residual_grid = griddata(points, residuals, (P1_grid, P2_grid), 
                               method='linear', fill_value=0)
        
        # Добавляем интерполированные остатки к основной поверхности
        # Масштабируем влияние остатков в зависимости от α
        roughness_intensity = 0.5 + alpha * 0.5  # Интенсивность от 0.5 до 1.0
        P3_pred_grid = P3_pred_grid + residual_grid * roughness_intensity
        
        # Альтернативный метод: добавляем шум, основанный на стандартном отклонении остатков
        if np.any(np.isnan(residual_grid)):
            # Если интерполяция не покрыла все области, добавляем случайный шум
            std_residuals = np.std(residuals)
            noise = np.random.normal(0, std_residuals * 0.3 * roughness_intensity, P3_pred_grid.shape)
            nan_mask = np.isnan(residual_grid)
            P3_pred_grid[nan_mask] = P3_pred_grid[nan_mask] + noise[nan_mask]
    ...
    # Настройки поверхности в зависимости от режима
    if smooth_surface:
        ...
        # Несглаженная поверхность с неровностями
        surface_trace = go.Surface(
            x=P1_grid, y=P2_grid, z=P3_pred_grid,
            colorscale='Plasma',
            opacity=0.8,
            name=f'Полином {degree}-й степени с неровностями',
            showscale=False,
            # Настройки для лучшего отображения неровностей
            lighting=dict(
                ambient=0.4,
                diffuse=0.9,
                specular=0.1,
                roughness=0.8  # Увеличиваем шероховатость для подчеркивания неровностей
            ),
            lightposition=dict(x=100, y=200, z=500),
            # Добавляем контуры для лучшего восприятия глубины
            contours=dict(
                z=dict(
                    show=True,
                    usecolormap=True,
    ...
    surface_mode = "сглаженная" if smooth_surface else "с неровностями"
    
    fig.update_layout(
        title=f'Регрессионная модель П₃ = φ(П₁, П₂) (α={alpha}, степень={degree}, {surface_mode})',
        scene=dict(
            xaxis_title='P1',
            yaxis_title='P2', 
            zaxis_title='P3',
...
def show_comprehensive_instructions():
    """Обновленная инструкция с правильным объяснением"""
    ...
    with st.expander("🎯 Цель лабораторной работы", expanded=False):
        ...
        <h3>🎯 Ожидаемые диапазоны значений БЕЗ погрешности</h3>
        <ul>
            <li><strong>P1</strong>: от 0 до 4 (равномерное распределение)</li>
            <li><strong>P2</strong>: от 0 до 8 (распределение Эрланга)</li>
            <li><strong>P3</strong>: от -9 до 9 (нормальное распределение)</li>
        </ul>
        
        <h3>📈 Влияние погрешности измерений</h3>
        <p>При добавлении погрешности ε_i ~ N(0, α·σ_i) диапазоны значений закономерно расширяются:</p>
        <ul>
            <li>α = 0.1: малая погрешность, небольшие отклонения</li>
            <li>α = 0.5: средняя погрешность, заметное расширение диапазонов</li>
            <li>α = 1.0: погрешность равна СКО распределения</li>
            <li>α = 1.5: высокая погрешность, значительное расширение диапазонов</li>
        </ul>
        
        <h3>🌊 Особенности визуализации поверхности</h3>
        <p><strong>Сглаживание поверхности:</strong></p>
        <ul>
            <li><strong>Включено</strong> - поверхность строится по регрессионной модели и выглядит гладкой</li>
            <li><strong>Выключено</strong> - поверхность строится методом интерполяции исходных данных, 
            что создает реалистичные неровности, "бугорки" и "впадины", отражающие реальное распределение данных</li>
        </ul>
        <p>При увеличении параметра α (погрешности) неровности становятся более выраженными, 
        особенно при отключенном сглаживании.</p>
        </div>
...
def main():
    ...
    st.sidebar.markdown('<div class="sidebar-header">🌊 Настройки поверхности</div>', unsafe_allow_html=True)
    
    # Переключатель сглаживания поверхности
    smooth_surface = st.sidebar.checkbox(
        "Сглаживать поверхность", 
        value=st.session_state.smooth_surface,
        help="При отключении сглаживания поверхность будет показывать реалистичные неровности на основе интерполяции исходных данных"
    )
    
    if smooth_surface != st.session_state.smooth_surface:
        st.session_state.smooth_surface = smooth_surface
        st.rerun()
    
    # Размер сетки поверхности
    grid_size = st.sidebar.slider(
        "Размер сетки поверхности", 
        50, 200, 100, 10,
        help="Количество точек по каждой оси для построения поверхности. Больше точек = лучше видны неровности, но медленнее расчет."
    )
    
    # Максимальная степень полинома
    max_poly_degree = st.sidebar.slider(
        "Максимальная степень полинома", 
        1, 2, 2, 1,
        help="Максимальная степень полинома для регрессионной модели. По замечанию профессора - не более 2."
    )
        ...
        if not alphas:
            ...
            with st.spinner("Выполняются расчеты..."):
                ...
                for i, alpha in enumerate(alphas):
                    ...
                    best_degree, best_model = select_polynomial_degree(X, y, max_degree=max_poly_degree)
    ...
    if st.session_state.results is not None:
        ...
        with tab1:
            ...
            # Показываем диапазоны данных с объяснением
            st.markdown("**Фактические диапазоны данных с учетом погрешности:**")
            for result in results:
                ranges_info, explanation = validate_data_ranges(result['P1'], result['P2'], result['P3'], result['alpha'])
                st.text(ranges_info)
            ...
            **Особенности визуализации поверхности:**
            - Режим сглаживания: **{'включен' if st.session_state.smooth_surface else 'выключен'}**
            - Размер сетки: **{grid_size}×{grid_size}** точек
            - Максимальная степень полинома: **{max_poly_degree}**

            **Рекомендации по анализу неровностей:**
            - При отключенном сглаживании поверхность строится методом интерполяции исходных данных
            - Это создает реалистичные неровности, отражающие реальное распределение данных
            - При больших значениях α неровности становятся более выраженными
            - Для анализа "бугорков" и "впадин" рекомендуется использовать режим без сглаживания
            """
        ...
        with tab3:
            ...
            # Информация о настройках поверхности
            st.info(f"""
            **Текущие настройки поверхности:**
            - Сглаживание: **{'включено' if st.session_state.smooth_surface else 'выключено'}**
            - Размер сетки: **{grid_size}×{grid_size}** точек
            - Максимальная степень полинома: **{max_poly_degree}**

            **Особенности режимов:**
            - **Сглаживание включено**: поверхность строится по регрессионной модели (гладкая)
            - **Сглаживание выключено**: поверхность строится методом интерполяции исходных данных (неровная)
            """)
            ...
            for result in results:
                ...
                fig_3d = create_3d_regression_plot(
                    result['P1'], result['P2'], result['P3'],
                    result['model'], alpha, degree, grid_size, st.session_state.smooth_surface
                )
                ...
                # Анализ неровностей для данного α
                st.markdown(f"**Анализ поверхности при α={alpha}:**")
                if not st.session_state.smooth_surface:
                    if alpha > 0.5:
                        st.markdown("✅ **Выраженные неровности**: При таком уровне погрешности и отключенном сглаживании поверхность демонстрирует реалистичные неровности, отражающие разброс данных.")
                    else:
                        st.markdown("✅ **Умеренные неровности**: При малой погрешности неровности менее выражены, но все же видны благодаря интерполяции исходных данных.")
                else:
                    st.markdown("ℹ️ **Гладкая поверхность**: Включенный режим сглаживания создает гладкую поверхность на основе регрессионной модели.")

...
def create_3d_regression_plot(P1, P2, P3, model, alpha, degree, grid_size=100, show_roughness=True, roughness_factor=0.1):
    """
    Создание 3D визуализации регрессионной поверхности с ярко выраженными неровностями
    
    Parameters:
    P1, P2, P3: исходные данные
    model: обученная регрессионная модель
    alpha: уровень погрешности
    degree: степень полинома
    grid_size: размер сетки (чем больше, тем детальнее поверхность)
    show_roughness: показывать неровности
    roughness_factor: интенсивность неровностей
    """
    ...
    # ДОБАВЛЯЕМ НЕРОВНОСТИ для демонстрации (если включено)
    if show_roughness and alpha > 0:
        # Добавляем шум, пропорциональный уровню погрешности α
        noise = np.random.normal(0, roughness_factor * alpha, P3_pred_grid.shape)
        P3_pred_grid += noise
    ...
    fig.add_trace(go.Surface(
        ...
        showscale=False,
        # Настройки освещения для подчеркивания неровностей
        lighting=dict(
            ambient=0.4,  # Уменьшаем общее освещение
            diffuse=0.9,  # Увеличиваем рассеянный свет
            specular=0.3, # Уменьшаем блики
            roughness=0.8 # Увеличиваем шероховатость
        ),
        lightposition=dict(x=100, y=200, z=500), # Меняем позицию света
        # Настройки контуров для лучшего восприятия глубины
        contours=dict(
            z=dict(
                show=show_roughness,  # Показываем контуры только в режиме неровностей
                usecolormap=True,
    ...
    surface_mode = "с неровностями" if show_roughness else "сглаженная"
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            zaxis_title='P3',
            bgcolor=bg_color,
            # Настройки камеры для лучшего обзора неровностей
            camera=dict(
                eye=dict(x=1.8, y=1.8, z=1.2)  # Изменяем угол для лучшего обзора рельефа
            ),
            # Настройки осей для лучшего восприятия
            xaxis=dict(gridcolor='lightgray' if st.session_state.theme == "light" else 'gray'),
            yaxis=dict(gridcolor='lightgray' if st.session_state.theme == "light" else 'gray'),
            zaxis=dict(gridcolor='lightgray' if st.session_state.theme == "light" else 'gray')
        ),
        template=template,
        height=600,
        font=dict(
...
def show_comprehensive_instructions():
    ...
    with st.expander("🎯 Цель лабораторной работы", expanded=False):
        ...
        <ul>
            <li><strong>Включено</strong> - поверхность выглядит гладкой, подходит для общей визуализации</li>
            <li><strong>Выключено</strong> - поверхность показывает реальные неровности, "бугорки" и "впадины", 
            что важно для анализа локальных особенностей регрессионной модели. Используются контуры и специальное освещение для лучшего отображения деталей.</li>
        </ul>
...
def main():
    ...
    # НАСТРОЙКИ ПОВЕРХНОСТИ - ДОБАВЛЕНЫ ПО ЗАМЕЧАНИЮ ПРОФЕССОРА
    ...
    smooth_surface = st.sidebar.checkbox(
        ...
        help="При отключении сглаживания поверхность будет показывать реальные неровности, 'бугорки' и 'впадины'"
    )
    ...
    if st.session_state.results is not None:
        ...
        with tab1:
            ...
            **Рекомендации по анализу неровностей:**
            - При отключенном сглаживании лучше видны локальные особенности поверхности
            - Увеличение размера сетки улучшает детализацию неровностей
        ...
        with tab3:
            ...
            **Рекомендации:** Для лучшего отображения неровностей отключите сглаживание и увеличьте размер сетки.
            """)
            ...
            for result in results:
                ...
                st.markdown(f"**Анализ неровностей поверхности при α={alpha}:**")
                if alpha > 0.5:
                    st.markdown("При таком уровне погрешности неровности поверхности должны быть хорошо заметны, особенно при отключенном сглаживании.")
                else:
                    st.markdown("При малой погрешности неровности поверхности менее выражены, но все же могут быть видны при детальном рассмотрении.")
